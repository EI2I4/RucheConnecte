...
// capteur de vent 
static     DavisAnemometer anemometer(A1 /* wind direction */, D13 /* wind speed */);
float      anemo;
...
int main() {
    ...
    while(1) {
        ...
        // myLED = 0 pour indiqué etat OFF
        //pc.printf("\nBoutton non appuie\n");
        myLED = 0;
        if(TriggerBoutton){
                // Des qu'on detecte une appuie de boutton, la LED s'allume pour indiqué un etat ON
                myLED = 1;
                pc.printf("\nBoutton appuie\n");
                ...
                // si le boutton n'est pas sur ON, on ettient la LED pour indiqué un etat OFF
                pc.printf("\nBoutton non appuie\n");
                myLED = 0;
                }
        }
